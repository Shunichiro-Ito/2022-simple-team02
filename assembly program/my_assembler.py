    ...
    for i in range(len(data)):
        ...
        try:
            if cmd == "ADD":
                ...
                result.append(to_binary(args[0], 16, signed=True))
            elif cmd == "ADDI":
                result.append(
                    "10" 
                    + "001" 
...
def write_result(result):
    ...
    if len(sys.argv) >= 3:
        ...
        fout.write("WIDTH=16;\n")
        fout.write("DEPTH=4096;\n")
        fout.write("ADDRESS_RADIX=HEX;\n")
        fout.write("DATA_RADIX=DEC;\n")
        fout.write("CONTENT BEGIN\n")
        for i in range(len(result)):
            #fout.write("\t" + str(i) + " : " + result[i] + ";\n")
            fout.write("\t" + format(i,  'x') + ":" + str(int(result[i], 2)) + ";\n" ) 
        fout.write("END;\n")
        fout.close()
    else:
        print("WIDTH=16;")
        ...
        for i in range(len(result)):
            ...
            print("\t" + format(i,  'x') + ":" + str(int(result[i], 2)) + ";" ) 
        print("END;")
